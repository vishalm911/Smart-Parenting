...
const ParentLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    if (!account) {
      // New user — create account with parent role
      await createUserAccount(result.user.uid, {
        email: result.user.email,
        role: 'parent',
        displayName: result.user.displayName || '',
      });
      // AuthContext onAuthChange is already running; isRoleReady spinner handles the gap
      navigate('/parent/dashboard');
      return;
    }

    // Route to their actual role dashboard (handles mis-navigation gracefully)
    const destination = ROLE_DASHBOARDS_MAP[account.role] || '/parent/dashboard';
  ...
  const handleGoogleLogin = async () => {
    ...
    const destination = ROLE_DASHBOARDS_MAP[existing.role] || '/parent/dashboard';
    navigate(destination);
  };
        ...
        <form onSubmit={handleLogin}>
          ...
          <Button
            ...
            sx={{
              ...
              '&:hover': { boxShadow: '0 10px 28px rgba(59,183,126,0.55)' },
            }}
          >
